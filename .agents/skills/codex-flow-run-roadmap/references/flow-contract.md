...
- `run-roadmap` kończy implementację walidacją integracji całego wyniku. Wszystkie jego etapy, również skille podrzędne, wykonuje główny agent w jednym wątku, bez subagentów, stagingu, commitów i pusha.
- Niezależne review oznacza ocenę przez osobnego agenta, który nie implementował zmiany. Jest dostępne poza `run-roadmap` wyłącznie na jawne zlecenie użytkownika.
- Milestone i cała roadmapa wymagają adekwatnej walidacji. Review jest wymagane wyłącznie na jawne zlecenie użytkownika. Jawnie zamówione review jest częścią kryteriów zakończenia zadania.
