...
5. Porównaj dokumentację i checkpoint z faktycznym stanem repo oraz historią Git. Sprawdź stałą bazę `review_base`, fazę przebiegu, wyniki milestone'ów, zmienione i nowe pliki, review całego zakresu i łączną liczbę rund poprawek. `implemented_pending_review` oznacza zakończoną implementację oczekującą na zbiorcze review: nie implementuj jej ponownie i nie traktuj jako zatwierdzonej. Zachowaj bazę i licznik rund przy wznowieniu. Wznawiaj rozpoczętą pracę; nie powtarzaj zatwierdzonego milestone'u z powodu starego statusu. Nie oczekuj commitów z `run-roadmap`: wynik i checkpoint pozostają w worktree. Jeśli sesja urwała się przed checkpointem, ustal pozostały zakres z diffu i kodu. Nie zgaduj brakującego wyniku review.
