...
2. Ustal bazę i uzgodniony zakres. Stosuj sekcje kontraktu: [review i zakończenie pracy](../codex-flow-run-roadmap/references/flow-contract.md#review-i-zakończenie-pracy), [własność zmian i zakres review](../codex-flow-run-roadmap/references/flow-contract.md#własność-zmian-i-zakres-review) oraz [aktualność oceny](../codex-flow-run-roadmap/references/flow-contract.md#aktualność-oceny). Pozostałe sekcje doczytuj tylko wtedy, gdy wymaga tego bieżąca decyzja. W `run-roadmap` oceniaj cały przebieg, nie tylko ostatni milestone. Sprawdź zakres przed i po ocenie; wynik dotyczy konkretnej treści, także nowych plików.
...
Zwróć również bazę, ocenione ścieżki i fingerprint. Prowadzący workflow zapisuje wynik i statusy; reviewer nie edytuje checkpointu.
